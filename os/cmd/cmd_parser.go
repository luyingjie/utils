...
func ParseWithArgs(args []string, supportedOptions map[string]bool, strict ...bool) (*Parser, error) {
	...
	for i := 0; i < len(args); {
		if option := parser.parseOption(args[i]); option != "" {
			array, _ := regex.MatchString(`^(.+?)=(.+)$`, option)
			if len(array) == 3 {
...
func (p *Parser) parseOption(argument string) string {
	array, _ := regex.MatchString(`^\-{1,2}(.+)$`, argument)
	if len(array) == 2 {
		return array[1]
	}
	return ""
...
func (p *Parser) setOptionValue(name, value string) {
	for optionName, _ := range p.passedOptions {
		array := str.SplitAndTrim(optionName, ",")
		for _, v := range array {
...
func (p *Parser) GetOptVar(name string, def ...interface{}) *vvar.Var {
	if p.ContainsOpt(name) {
		return vvar.New(p.GetOpt(name))
	}
	if len(def) > 0 {
...
func (p *Parser) GetArgVar(index int, def ...string) *vvar.Var {
	return vvar.New(p.GetArg(index, def...))
}
