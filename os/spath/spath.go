// Package spath implements file index and search for folders.
//
...
type SPath struct {
	paths *array.StrArray // The searching directories array.
	cache *vmap.StrStrMap // Searching cache map, it is not enabled if it's nil.
}
...
var (
	...
	pathsMap = vmap.NewStrAnyMap(true)
)

// New creates and returns a new path searching manager.
func New(path string, cache bool) *SPath {
	sp := &SPath{
		paths: array.NewStrArray(true),
	}
	if cache {
		sp.cache = vmap.NewStrStrMap(true)
	}
	if len(path) > 0 {
...
func (sp *SPath) Set(path string) (realPath string, err error) {
	...
	if realPath == "" {
		...
		if realPath == "" {
			realPath = file.RealPath(file.Pwd() + file.Separator + path)
		}
	}
	if realPath == "" {
	...
	if file.IsDir(realPath) {
		realPath = strings.TrimRight(realPath, file.Separator)
...
func (sp *SPath) Search(name string, indexFiles ...string) (filePath string, isDir bool) {
	...
	if sp.cache == nil {
		...
		if len(indexFiles) > 0 && isDir {
			...
			path := ""
			for _, _file := range indexFiles {
				path = filePath + file.Separator + _file
				if file.Exists(path) {
					filePath = path
					isDir = false
					break
