...
func (sp *SPath) nameFromPath(filePath, rootPath string) string {
	name := str.Replace(filePath, rootPath, "")
...
func (sp *SPath) addToCache(filePath, rootPath string) {
	...
	if idDir {
		if files, err := file.ScanDir(filePath, "*", true); err == nil {
			//fmt.Println("gspath add to cache:", filePath, files)
			for _, path := range files {
				sp.cache.SetIfNotExist(sp.nameFromPath(path, rootPath), sp.makeCacheValue(path, file.IsDir(path)))
			}
		} else {
...
// addMonitorByPath adds fsnotify monitoring recursively.
...
// removeMonitorByPath removes fsnotify monitoring of <path> recursively.
func (sp *SPath) removeMonitorByPath(path string) {
	if sp.cache == nil {
		return
	}
	_ = fsnotify.Remove(path)
}
