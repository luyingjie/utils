...
const (
	...
	BitLongFile                                            // 完整文件名称 /home/go/src/vic/server.go
...
func NewLog(out io.Writer, prefix string, flag int) *Logger {
	...
	log := &Logger{out: out, prefix: prefix, flag: flag, file: nil, debugClose: false, CalldDepth: 2}
		...
		if log.flag&(BitShortFile|BitLongFile) != 0 {
			...
			if log.flag&BitShortFile != 0 {
				...
				for i := len(file) - 1; i > 0; i-- {
					if file[i] == '/' {
						//找到最后一个'/'之后的文件名称  如:/home/go/src/vic.go 得到 "vic.go"
						short = file[i+1:]
						break
					}
				}
