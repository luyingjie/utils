...
func (s *SMTP) SendMail(from, tos, subject, body string, contentType ...string) error {
	...
	header := map[string]string{
		...
		"Subject":                   fmt.Sprintf("=?UTF-8?B?%s?=", contentEncoding.EncodeToString(conv.UnsafeStrToBytes(subject))),
		"MIME-Version":              "1.0",
	...
	message += "\r\n" + contentEncoding.EncodeToString(conv.UnsafeStrToBytes(body))
	return smtp.SendMail(
		address,
		smtp.PlainAuth("", s.Username, s.Password, server),
		from,
		tosArr,
		conv.UnsafeStrToBytes(message),
	)
}
