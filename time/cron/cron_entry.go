...
func (c *Cron) addEntry(pattern string, job func(), singleton bool, name ...string) (*Entry, error) {
	...
	entry := &Entry{
		...
		times:    vtype.NewInt(gDEFAULT_TIMES),
		Job:      job,
		Time:     time.Now(),
	}
	if len(name) > 0 {
		...
		entry.Name = "gcron-" + conv.String(c.idGen.Add(1))
	}
	...
	entry.entry = timer.AddEntry(time.Second, entry.check, singleton, -1, timer.STATUS_STOPPED)
...
// The running times limits feature is implemented by gcron.Entry and cannot be implemented by timer.Entry.
// gcron.Entry relies on timer to implement a scheduled task check for gcron.Entry per second.
func (entry *Entry) check() {
	if entry.schedule.meet(time.Now()) {
		...
		case STATUS_CLOSED:
			//log.Path(path).Level(level).Debugf("[gcron] %s(%s) %s removed", entry.Name, entry.schedule.pattern, entry.jobName)
			entry.Close()
		...
		case STATUS_RUNNING:
			...
			//log.Path(path).Level(level).Debugf("[gcron] %s(%s) %s start", entry.Name, entry.schedule.pattern, entry.jobName)
			defer func() {
				if err := recover(); err != nil {
					//log.Path(path).Level(level).Errorf("[gcron] %s(%s) %s end with error: %v", entry.Name, entry.schedule.pattern, entry.jobName, err)
				} else {
					//log.Path(path).Level(level).Debugf("[gcron] %s(%s) %s end", entry.Name, entry.schedule.pattern, entry.jobName)
				}
