...
func newSchedule(pattern string) (*cronSchedule, error) {
	...
	if match, _ := regex.MatchString(`(@\w+)\s*(\w*)\s*`, pattern); len(match) > 0 {
		key := strings.ToLower(match[1])
		if v, ok := predefinedPatternMap[key]; ok {
			pattern = v
		} else if strings.Compare(key, "@every") == 0 {
			if d, err := vtime.ParseDuration(match[2]); err != nil {
				return nil, err
			} else {
				return &cronSchedule{
	...
	// 0 0 0 1 1 2
	if match, _ := regex.MatchString(gREGEX_FOR_CRON, pattern); len(match) == 7 {
