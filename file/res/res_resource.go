...
func New() *Resource {
	return &Resource{
		tree: vtree.NewBTree(gDEFAULT_TREE_M, func(v1, v2 interface{}) int {
...
func (r *Resource) Load(path string, prefix ...string) error {
	...
	return r.Add(file.GetContents(realPath), prefix...)
}
...
func (r *Resource) doScanDir(path string, pattern string, recursive bool, onlyFile bool) []*File {
	...
	r.tree.IteratorFrom(path, true, func(key, value interface{}) bool {
		...
		for _, p := range patterns {
			if match, err := filepath.Match(p, file.Basename(name)); err == nil && match {
...
func (r *Resource) Dump() {
	...
	r.tree.Iterator(func(key, value interface{}) bool {
		...
		fmt.Printf("%v %7s %s\n", vtime.New(info.ModTime()).ISO8601(), file.FormatSize(info.Size()), key)
		return true
	})
