...
func Send(method string, params map[string]interface{}, conf map[string]interface{}) (interface{}, error) {
	...
	_params.Set("access_key_id", conf["console_key_id"].(string))
	...
	signature := qcutil.Get_iaas_authorization(conf["console_secrect_key"].(string), _method, conf["console_uri"].(string), urlParams)
	...
	if _method == "post" || _method == "put" {
		...
		headers["Content-Length"] = string(len(_data))
	}
